...
def get_user(user_id):
    ...
    response = requests.get(f"{base_url}/items/user/{user_id}", headers=headers)
    return response.json()

def add_item(new_item):
    """ Envoie une requête POST pour ajouter un nouvel item. """
    headers = {"Authorization": f"Bearer {token}"}
    response = requests.post(f"{base_url}/add-item/", json=new_item, headers=headers)
    return response.json()

def update_item(order_id, updated_item):
    """ Envoie une requête PUT pour mettre à jour un item. """
    headers = {"Authorization": f"Bearer {token}"}
    response = requests.put(f"{base_url}/update-item/{order_id}", json=updated_item, headers=headers)
    return response.json()

def delete_item(order_id):
    """ Envoie une requête DELETE pour supprimer un item. """
    headers = {"Authorization": f"Bearer {token}"}
    response = requests.delete(f"{base_url}/delete-item/{order_id}", headers=headers)
    return response.json()
